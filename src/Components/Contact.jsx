...
export default function Contact() {
  const socialLinks = [
    { name: 'GitHub', href: 'https://github.com/', icon: <FaGithub />, color: 'hover:bg-white hover:text-black' },
    { name: 'LinkedIn', href: 'https://linkedin.com/in/tawiah-obed-73941a3ab', icon: <FaLinkedinIn />, color: 'hover:bg-blue-600 hover:text-white' },
    { name: 'WhatsApp', href: 'https://wa.me/233539526814', icon: <FaWhatsapp />, color: 'hover:bg-green-500 hover:text-white' },
    { name: 'X', href: '#', icon: <FaTwitter />, color: 'hover:bg-sky-500 hover:text-white' },
    { name: 'Telegram', href: '#', icon: <FaTelegramPlane />, color: 'hover:bg-blue-400 hover:text-white' },
    { name: 'TikTok', href: '#', icon: <FaTiktok />, color: 'hover:bg-pink-600 hover:text-white' },
  ];
          ...
          >
            <motion.div variants={fadeInUp}>
              ...
              <h2 className="text-5xl lg:text-7xl font-bold text-white tracking-tighter leading-tight">
                ...
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-violet-500">extraordinary.</span>
              </h2>
            </motion.div>

            <motion.p variants={fadeInUp} className="text-xl text-slate-400 max-w-md leading-relaxed">
              Whether you have a question or just want to say hi, my inbox is always open. I'll get back to you within 24 hours.
            </motion.p>

            {/* Quick Contact Cards */}
            <motion.div variants={fadeInUp} className="space-y-4 pt-8">
              {[
                { icon: <FiMail />, label: 'Email', value: 'tawiaho858@gmail.com', href: 'mailto:tawiaho858@gmail.com', color: 'text-purple-400' },
                { icon: <FiPhone />, label: 'WhatsApp', value: '+233 53 952 6814', href: 'https://wa.me/233539526814', color: 'text-green-400' },
                { icon: <FiMapPin />, label: 'Location', value: 'Accra, Ghana', href: '#', color: 'text-cyan-400' },
              ].map((item, i) => (
                <a 
                  key={i} 
                  href={item.href}
                  className="group flex items-center p-4 rounded-2xl bg-white/[0.03] border border-white/5 hover:border-white/20 hover:bg-white/[0.06] transition-all duration-300"
                >
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-4">
              {socialLinks.map((link) => (
                ...
                >
                  {link.icon}
                  <span className="text-[10px] uppercase tracking-tighter font-bold">{link.name}</span>
                </motion.a>
              ))}
            </div>
            ...
            <div className="relative group overflow-hidden rounded-[2.5rem] border border-white/10 shadow-2xl">
              ...
              <div className="absolute bottom-4 left-4 z-20 bg-black/80 backdrop-blur-md border border-white/10 px-4 py-2 rounded-full">
                <p className="text-xs text-white/70 font-mono italic">Based in Accra, Ghana 🇬🇭</p>
              </div>
            </div>
          </motion.div>
